...
def clique_v2(filename, threshold):
    #自底向上       2017.5.26
    windowGraph = {}
    ...
    #weight_value = list(np.arange(0, 1, 0.05)) # [0,0.05,0.1,...0.9,0.95]
    weight_value.sort(reverse=True)
    print min(weight_value), max(weight_value)
    for t in weight_value :        
        for window in range(0, data.shape[1]) :
            window_term[window] = {}
            df = data[data[data.columns[window]]==t][data.columns[window]] #get each column(window)            
            #df = data[(data[data.columns[window]]>=t) & (data[data.columns[window]]<=t+0.05) ][data.columns[window]]
            ...
            num_clq = 0
            #print t,window,'找到clique数目：',len(list(nx.find_cliques(windowGraph[window])))
            ...
            print t,window,'term生成了：',len(dic_term)
    ...
    '''
    '''
    fw3 = open('term_annotation_v1.txt', 'w')
    # input the annotation of each term by nodes attribute---annotation
    for i in sorted(cliqueGraph.nodes()):
        try :
            fw3.write(str(i) + '\t'+cliqueGraph.node[i]['windowsize']+'\t'+ str(len(cliqueGraph.node[i]['annotation'])) + '\t'+ ' '.join(cliqueGraph.node[i]['annotation']) +'\n')
        except :
            continue
    fw3.close()
    '''
...
def clique_v3(filename, threshold):
    #自底向上       2017.6.2
    windowGraph = {}
    cliqueGraph = nx.DiGraph()    
    data = pd.read_csv(filename, index_col = 0, sep = '\t' )
    ...
    for t in weight_value :  
        ...
        for window in range(0, data.shape[1]) :
            ...
            # 当前 window 在 threshold=t 构建完成
            #print t,window,'找到clique数目：',len(list(nx.find_cliques(windowGraph[window])))
            if len(last_window_clique) == 0 : #第一个window
                last_window_clique = [i for i in sorted(list(nx.find_cliques(windowGraph[window])), key=lambda x: (len(x), x, x.sort()),reverse = True) if len(i)>3]
            else :                
                for clique in [i for i in sorted(list(nx.find_cliques(windowGraph[window])), key=lambda x: (len(x), x, x.sort()), reverse = True) if len(i)>3] : #cliques sorted by size of each clique
                    flag = 0                
                    if clique in last_window_clique :#合并
                        if clique not in dic_term.values(): #new term
                            print 'last_window',term,clique
                            dic_term[term] = clique
                            dic_term_w[term] = [window-1, window]
                            term = term +1
                        else :  # change term windowsize
                            
                            find_term = [k for k,v in dic_term.items() if v ==clique]
                            for i in find_term :                               
                                if max(dic_term_w[i])==window-1 :
                                    dic_term_w[i] = dic_term_w[i] + [window]
                                    flag = 1
                                    break
                            if flag==0:# 同一个clique但在不同的时期保持同步
                                dic_term[term] = clique
                                dic_term_w[term] = [window-1, window]
                                term = term +1
                    else:#查找公共部分
                        temp = []
                        limit_genesize = 3
                        for last_clique in last_window_clique :
                            intersection_gene = set(clique).intersection(last_clique)
                            print len(last_clique)
                            if len(intersection_gene) > limit_genesize :
                                temp = last_clique
                                limit_genesize = len(intersection_gene)
                        if temp :                            
                            if temp not in dic_term.values():
                                print 'temp new window',term, temp
                                dic_term[term] = temp
                                dic_term_w[term] = [window-1, window]
                                term = term +1
                            else :
...
if __name__ == '__main__':   
    ...
    clique_v3('G:\project2\\NPM201507\\code\\result_c5_s10_v2_weight.txt',1)
    end = time.clock()
